...
import { timeAgo, inboxGet } from "@/components/inbox/shared";
  ...
  const handleBulkAction = async (action: "send" | "approve" | "reject") => {
    ...
    try {
      await fetch("/api/inbox-proxy", {
        ...
        body: JSON.stringify({ path: "/api/drafts/bulk", body: { ids: Array.from(selected), action, acted_by: (session?.user as Record<string, unknown>)?.login as string || session?.user?.name || null } }),
      });
      clearSelection();
      fetchDrafts();
    } finally {
    ...
    <div className="space-y-5">
      ...
      {drafts.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap overflow-x-auto pb-1">
          <span className="text-[11px] uppercase tracking-wider text-zinc-500 mr-1 shrink-0">Filter</span>
...
function DraftCard({
  ...
  const { data: session } = useSession();
  const actedBy = (session?.user as Record<string, unknown>)?.login as string || session?.user?.name || null;
  ...
  const handleAction = async (action: "approve" | "reject" | "send") => {
    ...
    try {
      ...
      await fetch("/api/inbox-proxy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ path: endpoint, body: { id: draft.id, acted_by: actedBy } }),
      });
      onAction();
    } finally {
      setSubmitting(false);
