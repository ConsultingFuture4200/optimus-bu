...
export default function SettingsPage() {
  ...
  const refresh = useCallback(() => {
    ...
    fetch(`${API_URL}/api/voice/status`, { signal: AbortSignal.timeout(8000) })
      ...
      .then(setVoiceStatus)
      .catch(() => {});
    fetch(`${API_URL}/api/drive/watches`, { signal: AbortSignal.timeout(8000) })
      .then((r) => r.json())
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-48 rounded-md bg-surface-overlay border border-white/10 px-3 py-2 text-sm text-white placeholder:text-zinc-500 focus:border-accent focus:outline-none"
              />
            </div>
            <div className="flex items-center gap-2">
              <select
